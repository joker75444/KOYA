--[[
   ...
   ███████     ██              ██      ███████     ██   █
                                                                ███
BLANK ✾ @eljo_ker_m
تم تطوير وبرمجة السورس من قبل الجوكر ✾ 
 @eljo_ker_m
...
blank = Token:match("(%d+)")
redis:set(blank..":VERSION",1)
redis:set(blank..":SUDO_ID:",SUDO_USER)
redis:set(blank..":DataCenter:",'German')
redis:set(blank..":UserNameBot:",BOT_User)
redis:set(blank..":NameBot:",BOT_NAME)
redis:hset(blank..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3blank_INSTALL","Yes")
info = {}
...
echo -e "\e[38;5;77m       ✾تم تطوير وبرمجة السورس من قبل الجوكر ✾ .e[0m"
...
blank = Token:match("(%d+)")
our_id = tonumber(blank)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(blank..":UserNameBot:")
SUDO_ID = tonumber(redis:get(blank..":SUDO_ID:"))
SUDO_USER = redis:hgetall(blank..'username:'..SUDO_ID).username
version = redis:get(blank..":VERSION")
DataCenter = redis:get(blank..":DataCenter:")
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس كويا بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @UP_II }• }•\e[0m"
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..blank..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(blank..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(blank..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(blank..'group:add'..msg.chat_id_) then 
...
elseif msg.sender_user_id_ == 1519444978 then 
...
elseif msg.sender_user_id_ == 1669332184 then 
msg.TheRank =  ' مطور السورس '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(blank..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(blank..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(blank..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(blank..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(blank..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(blank..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(blank..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(blank..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(blank..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(blank..'username:'..SUDO_ID).username
...
• قنـاة سورس كويا  ⇐ @KOYA_SOURCE. مـعرف المطور  :]]..SUDO_USER:gsub([[\_]],'_')..[[ .
]])
return false
end
if not msg then
...
if msg.GroupActive and redis:get(blank..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(blank..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(blank..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(blank..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.blank then 
if msg.text and ScriptFile.iblank then
for k, blank in pairs(ScriptFile.blank) do
local SearchText = msg.text:match(blank)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iblank(msg,{msg.text:match(blank)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",blank.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dblank then
if ScriptFile.dblank(msg) == false then
return false
end
...
if Plug.blank then 
if msg.text and Plug.iblank then
for k, blank in pairs(Plug.blank) do
local SearchText = msg.text:match(blank)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(blank..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(blank..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iblank(msg,{msg.text:match(blank)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",blank.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source blank")
end 

end
else
print("The File Script.lua Not Runing in The Source blank")
...
function tdcli_update_callback(data)
	...
	redis:srem(blank..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE BLA')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/blank/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE BLANK\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس كويا بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(blank..":SUDO_ID:")
redis:hset(blank..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(blank..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(blank..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(blank..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(blank..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(blank..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(blank..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(blank..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(blank..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(blank..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(blank..'group:add'..data.chat_.id_) then
	redis:set(blank..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(blank..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(blank..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(blank..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	blank..'group:name'..k,IDS.Title,
	blank..'num_msg_blank'..k,5,
	blank..'group:add'..k,true,
	blank..'lock_link'..k,true,
	blank..'lock_spam'..k,true,
	blank..'lock_webpage'..k,true,
	blank..'lock_markdown'..k,true,
	blank..'lock_flood'..k,true,
	blank..'lock_bots'..k,true,
	blank..'mute_forward'..k,true,
	blank..'mute_contact'..k,true,
	blank..'mute_document'..k,true,
	blank..'mute_inline'..k,true,
	blank..'lock_username'..k,true,
	blank..'replay'..k,true
	)
	redis:sadd(blank..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(blank..'username:'..ID,'username',user)
	redis:sadd(blank..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(blank..'username:'..ID,'username',user)
	redis:sadd(blank..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(blank..'username:'..ID,'username',user)
	redis:sadd(blank..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(blank..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(blank..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(blank..'username:'..data.user_.id_).username
	...
	redis:hset(blank..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
